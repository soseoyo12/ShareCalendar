class CalendarScheduler {
    ...
    toggleTimeSlot(slot) {
        ...
        // timeSlots 구조 안전하게 초기화
        if (!this.scheduleData.timeSlots) {
            this.scheduleData.timeSlots = {};
        }
    ...
    calculateBestTimes() {
        ...
        allDates.forEach(date => {
            ...
            for (let hour = 9; hour < 24; hour++) {
                // 해당 시간에 가능한 참여자 수 계산
                let hourAvailableCount = 0;
                this.scheduleData.participants.forEach(participant => {
                    if (participant.schedule && 
                        participant.schedule[date] && 
                        participant.schedule[date][hour] === 'available') {
                        hourAvailableCount++;
                    } else if (participant.schedule &&
                               participant.schedule[date] &&
                               participant.schedule[date][hour] === undefined) {
                        hourAvailableCount++;
                    }
                });
                
                // 모든 참여자가 가능한 시간만 연속으로 계산
                const allAvailable = hourAvailableCount === this.scheduleData.participants.length;
                
                if (allAvailable) {
                    if (currentStart === null) {
                        currentStart = hour;
                        consecutiveCount = 1;
                        availableCount = hourAvailableCount;
                    } else {
                        consecutiveCount++;
                    }
                } else {
                    // 연속 구간이 끝났을 때 저장 (2시간 이상인 경우)
                    if (currentStart !== null && consecutiveCount >= 2) {
                        bestTimes.push({
                            date: date,
                            startHour: currentStart,
                            endHour: currentStart + consecutiveCount - 1,
        ...
        // 지속 시간 순으로 정렬 (긴 시간 우선)
        bestTimes.sort((a, b) => b.duration - a.duration);
        ...
        if (bestTimes.length === 0) {
            container.innerHTML = '<div class="no-times">모든 참여자가 가능한 연속 시간이 없습니다.</div>';
            return;
        }
            ...
            item.innerHTML = `
                <div class="time-info">
                    <span class="time-slot-info">
                        ${this.formatDate(new Date(timeSlot.date))} ${timeSlot.startHour}:00-${timeSlot.endHour + 1}:00
                    </span>
                    <span class="duration-info">
                        ${timeSlot.duration}시간 연속
                    </span>
    ...
    generateHeatmap() {
        ...
        for (let hour = 9; hour < 24; hour++) {
            ...
            dates.forEach(dateInfo => {
                ...
                this.scheduleData.participants.forEach(participant => {
                    // 안전한 접근 - schedule 존재 확인
                    if (participant.schedule && 
                        participant.schedule[dateInfo.key] && 
                        participant.schedule[dateInfo.key][hour] === 'available') {
                        availableCount++;
                    } else if (participant.schedule &&
                               participant.schedule[dateInfo.key] &&
                               participant.schedule[dateInfo.key][hour] === undefined) {
                        // undefined인 경우 available로 간주
                        availableCount++;
                    }
                });
    ...
    startSelection(e, slot) {
        ...
        // 전역 endDrag 리스너 임시 제거 (충돌 방지)
        if (this.boundEndDrag) {
            document.removeEventListener('mouseup', this.boundEndDrag);
            console.log('🔧 웹: 드래그 시작 - 전역 mouseup 리스너 임시 제거');
        }
        
        // 선택용 이벤트 리스너 추가 (바인딩된 함수 참조 저장)
        ...
        console.log('🔧 웹: 선택 시작 - 선택용 리스너 추가 완료');
    }
    ...
    endSelection() {
        ...
        try {
            ...
            // 전역 endDrag 리스너 다시 추가 (선택 완료 후 복원)
            if (this.boundEndDrag) {
                document.addEventListener('mouseup', this.boundEndDrag);
                console.log('🔧 웹: 선택 완료 - 전역 mouseup 리스너 복원');
            }
...
💡 위 링크로 추가 참여자를 초대할 수 있습니다!`;
    ...
    saveDataToFirebase() {
        ...
        if (this.scheduleData.timeSlots) {
            for (const [date, daySlots] of Object.entries(this.scheduleData.timeSlots)) {
                if (daySlots && typeof daySlots === 'object') {
                    cleanTimeSlots[date] = {};
                    for (const [hour, status] of Object.entries(daySlots)) {
                        if (status !== undefined && status !== null) {
                            cleanTimeSlots[date][hour] = status;
                        }
                    }
                }
            }
    ...
    saveParticipantToFirebase(participant) {
        ...
        const participantData = {
            ...
            schedule: participant.schedule || {}, // 시간 슬롯 데이터 포함
            showDetails: participant.showDetails || true,
        ...
        // 참여자 정보 저장 (iOS와 동일한 경로) - update 사용하여 기존 참여자 유지
        sessionRef.child(`participants/${userId}`).update(participantData)
            .then(() => {
        ...
        // timeSlots 별도 저장 제거 - participants/{name}/schedule만 사용 (iOS와 통일)
        
        // 날짜 범위도 저장 (iOS 호환을 위해 scheduleData/dateRange 경로 사용)
        if (this.scheduleData.dateRange.start && this.scheduleData.dateRange.end) {
            ...
            const dateFormatter = new Intl.DateTimeFormat('en-CA'); // YYYY-MM-DD 형식
            const dateRangeData = {
                start: dateFormatter.format(startDate), // iOS에서 문자열 형식으로 기대
                end: dateFormatter.format(endDate)
            };
            console.log('🔥 웹: 저장할 날짜 범위 (iOS 호환):', dateRangeData);
            
            // iOS와 호환되는 경로로 저장: scheduleData/dateRange
            sessionRef.child('scheduleData/dateRange').set(dateRangeData)
                .then(() => {
                    console.log('✅ 웹: scheduleData/dateRange 저장 성공');
                })
                .catch((error) => {
                    console.error('❌ 웹: scheduleData/dateRange 저장 실패:', error);
                });
        }
        
        // 통합 결과 생성 (iOS 호환) - 저장 완료 후 실행
        setTimeout(() => {
            this.updateAggregatedTimeSlotsFromParticipants();
        }, 500);
    }
    ...
    setupRealtimeListeners() {
        ...
        // 날짜 범위 실시간 감지 (iOS 호환 경로 사용)
        sessionRef.child('scheduleData/dateRange').on('value', (snapshot) => {
            if (snapshot.exists()) {
                const dateRange = snapshot.val();
                // iOS에서 이미 YYYY-MM-DD 문자열로 저장되므로 직접 사용
                const startDate = new Date(dateRange.start);
                const endDate = new Date(dateRange.end);
                
                document.getElementById('startDate').value = dateRange.start; // 이미 문자열
                document.getElementById('endDate').value = dateRange.end; // 이미 문자열
                this.scheduleData.dateRange.start = dateRange.start;
                this.scheduleData.dateRange.end = dateRange.end;
                this.generateTimeGrid();
                console.log('날짜 범위 업데이트 (iOS 호환):', dateRange);
            }
        });
    ...
    // 참여자 schedule 데이터에서 통합 결과 생성 (iOS 호환)
    updateAggregatedTimeSlotsFromParticipants() {
        if (typeof database === 'undefined') {
            return;
        }
        ...
        // 모든 참여자의 schedule 데이터를 가져와서 통합 (iOS와 동일한 로직)
        sessionRef.child('participants').once('value')
            .then((snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    console.log('🔥 웹: 참여자별 데이터:', participantsData);
                    
                    // 각 참여자의 schedule 데이터를 통합
                    for (const [participantName, participantData] of Object.entries(participantsData)) {
                        console.log(`🔥 웹: 참여자 ${participantName} 데이터 처리:`, participantData);
                        
                        if (participantData.schedule) {
                            for (const [dateKey, daySlots] of Object.entries(participantData.schedule)) {
            ...
            sessionRef.once('value')
                .then((snapshot) => {
                    if (snapshot.exists()) {
                        ...
                        // 기본 세션 데이터 로드 (iOS 호환 경로 우선)
                        if (data.scheduleData && data.scheduleData.dateRange) {
                            // iOS 앱에서 저장한 경우: 이미 YYYY-MM-DD 문자열 형식
                            this.scheduleData.dateRange.start = data.scheduleData.dateRange.start;
                            this.scheduleData.dateRange.end = data.scheduleData.dateRange.end;
                            console.log('🔗 웹: scheduleData.dateRange 로드 (iOS 호환):', this.scheduleData.dateRange);
                        } else if (data.dateRange) {
                            // 기존 웹 앱에서 저장한 경우: 타임스탬프를 문자열로 변환
                            const startDate = new Date(data.dateRange.start);
                            const endDate = new Date(data.dateRange.end);
                            this.scheduleData.dateRange.start = startDate.toISOString().split('T')[0];
                            this.scheduleData.dateRange.end = endDate.toISOString().split('T')[0];
                            console.log('🔗 웹: dateRange 로드 (기존 웹 호환):', this.scheduleData.dateRange);
                        }
                        ...
                        // UI 업데이트 - 공유 링크로 접속한 경우 1단계에서 시작
                        if (this.scheduleData.participants.length > 0) {
                            // 공유 링크로 접속한 사용자는 1단계에서 본인 일정을 설정해야 함
                            this.showStep(1);
                            this.updateParticipantsList();
                            this.generateShareLink();
                        }
